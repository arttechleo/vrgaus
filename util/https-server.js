...
// SSL options with certificate files
const options = {
...
https
  .createServer(options, async function (request, response) {
    ...
    fs.readFile(filePath, async function (error, content) {
      if (error) {
        ...
        console.log("HTTPS(200) Request for " + filePath);
...
// Find the Ethernet IPv4 address
if (interfaces['Ethernet']) {
  const ethernetInterface = interfaces['Ethernet'].find(iface => iface.family === 'IPv4');
  if (ethernetInterface) {
    ipAddress = ethernetInterface.address;
  }
}

console.log(`HTTPS Server running at https://${host}:${port}`);
console.log(`Access locally at: https://localhost:${port}`);
console.log(`Access from other devices at: https://${ipAddress}:${port}`);
